...
プッシュ通知を送信するには、プロジェクトの認証キーを生成して参照する必要があります。

### iOS用

Apple社から提供される `AuthKey_XXXYYY.p8` 認証ファイルが必要です。
...
Firebaseプロジェクトからサーバーキーを取得し、Cloud Message API を設定する必要があります。

1. [このドキュメント](https://github.com/4d/4D-Mobile-App-Server/blob/main/Documentation/Conf_firebase.md) で説明されているように、Firebaseプロジェクトを設定してサーバーキーを取得します。

2. プッシュ通知メソッドでは、以下のステートメントでサーバーキーを参照します:
